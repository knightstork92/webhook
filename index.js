...
const { google } = require("googleapis"); // Dòng này thường an toàn
...
async function processNewFile(drive, file, db, admin) {
    ...
    // 🚨 SỬA LỖI CÚ PHÁP: Khai báo và khởi tạo biến mã đơn hàng
    let code = null; 
...
app.post("/drive-webhook", async (req, res) => {
    ...
    try {
        ...
        let lastTokenNumber = 0; // Khởi tạo an toàn
        ...
        if (
            ...
            newTokenNumber > MIN_VALID_TOKEN
        ) {
